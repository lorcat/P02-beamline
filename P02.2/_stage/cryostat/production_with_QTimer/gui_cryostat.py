...
(SIGNALREPORT, SIGNALFAILED, SIGNALSUCCESS, SIGNALCOMMAND) = ("reportChange", "reportFailed", "successDeviceOnline", "commandReadWrite")
...
(DEFAULTHEATPID, DEFAULTHEATAUTO, DEFAULTHEATSETTEMP, DEFAULTHEATSETPOWER) = (SETTINGON, SETTINGON, 305., 5.)
...
class CryoControl(QMainWindow):
    ...
    def initVars(self, app):
        ...
        # timer to update all Mercury iTC parameters
        self._maintimer = QTimer(self)
        self._maintimer.setInterval(MAINTIMERINTERVAL)
    ...
    def initEvents(self):
        ...
        self.connect(self.btnInit, SIGNAL("clicked()"), self.btn_initCryoDevice)

        # start stop timer - use it as thread alternative for updating values
        self.connect(self.btnStartStop, SIGNAL("clicked()"), self.btn_startStopMeasurement)

        # timer events
        self.connect(self._maintimer, SIGNAL("timeout()"), self.processMainTimerUpdate)
        ...
        self.connect(self.sbsett1, SIGNAL("valueChanged(double)"), func_callback)

        func_callback = lambda v="", sense=SENSE1, attr=ATTRSETHEATPOWER: self.processAttributeUpdate(v, sense, attr)
        self.connect(self.sbsetp1, SIGNAL("valueChanged(double)"), func_callback)
        ...
        self.connect(self.sbsett2, SIGNAL("valueChanged(double)"), func_callback)

        func_callback = lambda v="", sense=SENSE2, attr=ATTRSETHEATPOWER: self.processAttributeUpdate(v, sense, attr)
        self.connect(self.sbsetp2, SIGNAL("valueChanged(double)"), func_callback)
    ...
    def createCentralWidget(self):
        ...
        grid.addWidget(self.cmbdevice, 0, 0)
        grid.addWidget(self.btnInit, 0, 1)
        grid.addWidget(self.btnStartStop, 0, 2)
        grid.addWidget(self.tab, 1,0, 1, 3)
        grid.setColumnStretch(0, 100)

        # adjust control widgets
        font = QFont("Arial", 12)
        self.setWidgetsFont(font, self.cmbdevice, self.btnStartStop, self.btnInit)
        ...
        self.setWidgetsTooltips((self.btnInit, "Resets Mercury iTC settings: full stop of the heating devices, temperature set-point is set to 300K"),
                                ...
                                (self.cmbdevice, "Device selection, test of the access"))
    ...
    def createSetTab(self, tab, pal):
        ...
        self.sbsett1 = QDoubleSpinBox()
        self.sbsett2 = QDoubleSpinBox()
        ...
        # set power spin box
        self.sbsetp1 = QDoubleSpinBox()
        self.sbsetp2 = QDoubleSpinBox()
    ...
    def createDeviceComboBox(self):
        ...
        self.cmbdevice.addItem("   Check Tango Device", "")
    ...
    def btn_initCryoDevice(self):
        # create a special worker thread
        self._worker = WorkerThread(self.tango.initCryoDevice, (self.btnInit, self.btnStartStop))
        self._worker.start()

    # process start/stop measurement commands - QPushButton
    def btn_startStopMeasurement(self):
        if(self._maintimer.isActive()):     # timer is running - stop it
            self.btnStartStop.setText(BTNSTART)
            self._maintimer.stop()

        else:                               # timer is stopped
            self.btnStartStop.setText(BTNSTOP)
            #self._maintimer.setSingleShot(True)
            self._maintimer.start()
        return

    # main timer event - timeout()
    def processMainTimerUpdate(self):
    ...
    def fillUISense(self, sense, *widgets):
        ...
        for k in sense:
            ...
            elif(t is QDoubleSpinBox and not wdgt.hasFocus()):      # QDouble spinbox type - sets value - should not update if the window is being edited
                wdgt.setValue(value)
        return
    ...
    def makeAttributeUpdate(self, sense, attrname, attrvalue):
        ...
        # check if we need to stop main timer
...
class TangoObject(QObject):
    ...
    def checkDeviceOnlineStatus(self):
        ...
        if(bsuccess):
            ...
            if(bsuccess and QString(res).indexOf(SCPITESTRESPONSE)<0):
                errormsg = "for %s command device gives some stupid response %s "%(SCPITEST, res)
                bsuccess = False
    ...
    def resetHeater(self, dev):
        ...
        if(bsuccess):
            bsuccess = self.devSetHeaterPID(dev, DEFAULTHEATAUTO)
        if(bsuccess):
...
###
## class TabBar - for QTabWidget tab bar ctusomization
...
###
## class WorkerThread - to separate gui and time consuming operations
